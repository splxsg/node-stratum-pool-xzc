...
var generateOutputTransactions = function(poolRecipient, recipients, rpcData){
    ...
    var rewardToZcoinFounder = 200000000;
...
//cant obtain option in this function , in order to take the xzc route, check if the recipients == 5
   if(recipients.length == 5)
   {
        for (var i = 0; i < recipients.length; i++){
        var recipientReward = Math.floor(rewardToZcoinFounder);
         //console.log(recipientReward+" "+recipients[i].script.length+" "+recipients[i].script);
        txOutputBuffers.push(Buffer.concat([
...
exports.CreateGeneration = function(rpcData, publicKey, extraNoncePlaceholder, reward, txMessages, recipients){
    ...
    var scriptSigPart2 = util.serializeString('/mph/');
    //console.log("scriptSigPart1.length: "+scriptSigPart1.length);
    //console.log("extraNoncePlaceholder.length: "+extraNoncePlaceholder.length);
    //console.log("scriptSigPart2.length: "+scriptSigPart2.length);
